# sift_n_kmeans_multiple_images.py
import glob
import numpy as np
import math
...
images_left = glob.glob(img_path + 'test1_segmented_images_left_selected/*.PNG')
images_right = glob.glob(img_path + 'test1_segmented_images_right_selected/*.PNG')
original_images_left = glob.glob(img_path + 'test1_rectified_images_left_selected/*.PNG')
original_images_right = glob.glob(img_path + 'test1_rectified_images_right_selected/*.PNG')

images_left.sort()
images_right.sort()
original_images_left.sort()
original_images_right.sort()

for i, fname in enumerate(images_right):
    img2 = cv2.imread(images_right[i])
    img1 = cv2.imread(images_left[i])
    img4 = cv2.imread(original_images_right[i])
    img3 = cv2.imread(original_images_left[i])
    ...
    xy_array2 = []
    sc = 0
    for i1, (m1, m2) in enumerate(matches):
    ...
    res = cv2.drawMatchesKnn(img3, kpts1, img4, kpts2, matches, None, **draw_params)
    ...
    cv2.imwrite(img_path + 'cluster_final_distances/' + 'stereoimage_' + '{}'.format(i) + '.png', res)
